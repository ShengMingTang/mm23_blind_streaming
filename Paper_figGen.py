...
# [YC] start
# Change font
csfont = {'family':'Times New Roman', 'serif': 'Times' , 'size' : 24}
...
ax2.set_ylabel('PSNR')
plt.savefig(f'./figs/sample_user_{param["scene"]}.eps', bbox_inches = 'tight')
plt.show()
...
ax.set_ylabel('SSIM / VMAF')
ax2.set_ylabel('PSNR')
plt.savefig(f'./figs/sample_user_{param["scene"]}_aggregate.eps', bbox_inches = 'tight')
plt.show()
for metric in METRICS:
	print(f'\t{metric}: mean = {selected[metric].mean()}, std = {selected[metric].std()}')
print(selected['PSNR'].argmax(), selected['PSNR'].argmin())
...
# axes.yaxis.set_label_coords(-.1, .1)

ax1 = sns.lineplot(ax=axes[0], data=selected, x='Frame', y='PSNR (dB)', err_style='bars', markers=True, dashes=True)
ax1.yaxis.labelpad = 15
# ax.yaxis.set_label_coords(0.0, 0.0)
# plt.title('PSNR')
plt.subplot(312)

ax2 = sns.lineplot(ax=axes[1], data=selected, x='Frame', y='SSIM', err_style='bars', markers=True, dashes=True)
# ax2.yaxis.labelpad = 20
# plt.title('SSIM')
plt.subplot(313)

ax3 = sns.lineplot(ax=axes[2], data=selected, x='Frame', y='VMAF', err_style='bars', markers=True, dashes=True)
ax3.yaxis.labelpad = 15
# plt.title('VMAF')
# plt.show()

plt.savefig(f'./figs/sample_user_{param["scene"]}_aggregate_subplots.eps', bbox_inches = 'tight')
...
selected.reset_index(inplace=True, drop=True)
selected = selected.assign(N = lambda x: x.m*N_USERS*FPS)
selected['N'] = selected['N'].astype('int')
selected = selected.assign(Content = lambda x: x.scene)
# print(selected.shape)
...
ax = sns.barplot(data=selected, x='Content', y='PSNR', hue='N')
# sns.move_legend(ax, 'lower left')
ax.set(xlabel='', ylabel='PSNR (dB)')
plt.legend(title='', loc='lower left', fontsize=20)
# plt.xticks(range(0,91,30))
# plt.xlim((0,90))
# plt.ylim((0,33))
plt.savefig('./figs/m_PSNR.eps', bbox_inches = 'tight')
plt.show()

ax = sns.barplot(data=selected, x='Content', y='SSIM', hue='N')
# sns.move_legend(ax, 'lower left')
ax.set(xlabel='', ylabel='SSIM')
plt.legend(title='', loc='lower left', fontsize=20)
plt.savefig('./figs/m_SSIM.eps', bbox_inches = 'tight')
plt.show()

ax = sns.barplot(data=selected, x='Content', y='VMAF', hue='N')
# sns.move_legend(ax, 'lower left')
ax.set(xlabel='', ylabel='VMAF')
plt.legend(title='', loc='lower left', fontsize=20)
# plt.xticks(range(0,91,30))
# plt.xlim((0,90))
# plt.ylim((0,100))
plt.savefig('./figs/m_VMAF.eps', bbox_inches = 'tight')
plt.show()
#%%
# X
# ---------------------------------------------------------------------------- #
#               plot increaseing source views with pure m variant -- line      #
# ---------------------------------------------------------------------------- #
def scene_m_metric(df):
    for scene in SCENES:
        # print(f'{scene}:')
...
hue_order = ['House', 'Bigroom', 'Smallroom']
xticks = [8, 16, 24, 32, 40]

ax = sns.lineplot(data=selected, x='N', y='PSNR', hue='Content', hue_order=hue_order, markers=True, err_style='bars', marker= 'x', markersize=140)
# sns.move_legend(ax, 'lower left')
ax.set(ylabel='PSNR (dB)')
ax.set_xticks(xticks)
plt.legend(bbox_to_anchor=(-0.1, 1.0), title='', loc='lower left', fontsize=20, ncol=3, columnspacing=0.2)
# plt.xticks(range(0,91,30))
# plt.xlim((0,90))
# plt.ylim((0,33))
# sns.move_legend(ax, 'lower right')
plt.savefig('./figs/m_PSNR_line.eps', bbox_inches = 'tight')
plt.show()

ax = sns.lineplot(data=selected, x='N', y='SSIM', hue='Content', hue_order=hue_order, markers=True, err_style='bars', marker= 'x', markersize=140)
# sns.move_legend(ax, 'lower left')
ax.set(ylabel='SSIM')
ax.set_xticks(xticks)
plt.legend(bbox_to_anchor=(-0.1, 1.0), title='', loc='lower left', fontsize=20, ncol=3, columnspacing=0.2)
# sns.move_legend(ax, 'lower right')
plt.savefig('./figs/m_SSIM_line.eps', bbox_inches = 'tight')
plt.show()

ax = sns.lineplot(data=selected, x='N', y='VMAF', hue='Content', hue_order=hue_order, markers=True, err_style='bars', marker= 'x', markersize=140)
# sns.move_legend(ax, 'lower left')
ax.set(ylabel='VMAF')
ax.set_xticks(xticks)
plt.legend(bbox_to_anchor=(-0.1, 1.0), title='', loc='lower left', fontsize=20, ncol=3, columnspacing=0.2)
...
# plt.xlim((0,90))
# plt.ylim((0,100))
plt.savefig('./figs/m_VMAF_line.eps', bbox_inches = 'tight')
plt.show()
...
hue_order = ['C2I', 'C2G', 'Uni', 'BB',  'UM', 'Opt']
selected = df[df.expDir.str.match(whichDir)]
print(selected.shape)
...
selected = selected.assign(Content = lambda x: x.scene)
selected = selected.assign(Solver = lambda x: x.solver)
...
# plt.xlim((0,90))
# plt.ylim((0,100))
# plt.savefig('./figs/TMM-slvr_SSIM.eps', bbox_inches='tight')
plt.show()

# ----------------------------------- VMAF ----------------------------------- #
ax = sns.barplot(data=selected, x='Content', y='VMAF', hue='Solver', hue_order=hue_order)
ax.set(xlabel='', ylabel='VMAF')
# plt.legend(bbox_to_anchor=(0.12, 1.0), ncol=3, columnspacing=1.0)
plt.legend(title='', loc='lower left', fontsize=20, ncol=3, columnspacing=0.7)
# plt.xticks(range(0,91,30))
# plt.xlim((0,90))
# plt.ylim((0,100))
# plt.savefig('./figs/TMM-slvr_VMAF.eps', bbox_inches='tight')
plt.show()
...
# plt.xlim((0,90))
# plt.ylim((0,100))
plt.savefig('./figs/IXR-slvr_PSNR.eps', bbox_inches = 'tight')
plt.show()
...
# plt.xlim((0,90))
# plt.ylim((0,100))
plt.savefig('./figs/IXR-slvr_SSIM.eps', bbox_inches = 'tight')
plt.show()
...
# plt.xlim((0,90))
# plt.ylim((0,100))
plt.savefig('./figs/IXR-slvr_VMAF.eps', bbox_inches = 'tight')
plt.show()

#%%
# Fig 10
# ---------------------------------------------------------------------------- #
#                             plot IXR/TMM - C2G,C2I,UM                        #
# ---------------------------------------------------------------------------- #
param = dict(DEFAULT_PARAM)
param['solverPolicy'] = '(UnB)'
param['psPolicy'] = '.*'
param['placePolicy'] = '.*'
param['m'] = 0.01
param['h'] = 0.15
...
hue_order = ['S-Cdd', 'Proposed']
selected = df[df.expDir.str.match(whichDir)]
print(selected.shape)
# print(N_USERS * FPS * VIDEO_LEN * len(hue_order) * N_SCENES)
print(selected.shape[0] / (N_USERS * FPS * VIDEO_LEN * N_SCENES * 2))
print(selected.expDir.unique())
assert selected.shape[0] == N_USERS * FPS * VIDEO_LEN * N_SCENES * 2
selected = selected.copy()
selected.solver.replace({
	'BB': 'BB',
	'UnB': 'UM',
}, inplace=True, regex=True)
selected.placePolicy.replace({
	'average': 'Proposed',
	'IXR': 'S-Cdd',
}, inplace=True, regex=True)
selected = selected.assign(Solver = lambda x: x.solver)
selected = selected.assign(Content = lambda x: x.scene)
selected = selected.assign(Cdd = lambda x: x.placePolicy)
# # scene_solver_metric(selected)
# '''
# House:
# solver: BB
...
# '''
# # ----------------------------------- PSNR ----------------------------------- #
ax = sns.barplot(data=selected, x='Content', y='PSNR', hue='Cdd', hue_order=hue_order)
# sns.move_legend(ax, 'lower left')
plt.rcParams['figure.dpi'] = 1200
ax.set(xlabel='', ylabel='PSNR (dB)')
# plt.legend(bbox_to_anchor=(0.12, 1.0), ncol=3, columnspacing=1.0)
...
# plt.xlim((0,90))
# plt.ylim((0,100))
plt.savefig('./figs/IXR-TMM-cdd_PSNR.eps', bbox_inches = 'tight')
plt.show()

# # ----------------------------------- SSIM ----------------------------------- #
ax = sns.barplot(data=selected, x='Content', y='SSIM', hue='Cdd', hue_order=hue_order)
# sns.move_legend(ax, 'lower left')
plt.rcParams['figure.dpi'] = 1200
ax.set(xlabel='', ylabel='SSIM')
# plt.legend(bbox_to_anchor=(0.12, 1.0), ncol=3, columnspacing=1.0)
...
# plt.xlim((0,90))
# plt.ylim((0,100))
plt.savefig('./figs/IXR-TMM-cdd_SSIM.eps', bbox_inches = 'tight')
plt.show()

# # ----------------------------------- VMAF ----------------------------------- #
ax = sns.barplot(data=selected, x='Content', y='VMAF', hue='Cdd', hue_order=hue_order)
# sns.move_legend(ax, 'lower left')
plt.rcParams['figure.dpi'] = 1200
ax.set(xlabel='', ylabel='VMAF')
# plt.legend(bbox_to_anchor=(0.12, 1.0), ncol=3, columnspacing=1.0)
...
# plt.xlim((0,90))
# plt.ylim((0,100))
plt.savefig('./figs/IXR-TMM-cdd_VMAF.eps', bbox_inches = 'tight')
plt.show()
...
sns.move_legend(ax, 'lower left')
plt.savefig('./figs/ffr_SSIM.eps', bbox_inches = 'tight')
plt.show()

ax = sns.barplot(data=selected, x='scene', y='VMAF', hue='ffr')
sns.move_legend(ax, 'lower left')
plt.savefig('./figs/ffr_VMAF.eps', bbox_inches = 'tight')
plt.show()

ax = sns.barplot(data=selected, x='scene', y='PSNR', hue='ffr')
sns.move_legend(ax, 'lower left')
plt.savefig('./figs/ffr_PSNR.eps', bbox_inches = 'tight')
plt.show()
# %%
# X
# ---------------------------------------------------------------------------- #
...
sns.move_legend(ax, 'lower left')
plt.savefig('./figs/a_SSIM.eps', bbox_inches = 'tight')
plt.show()

ax = sns.barplot(data=selected, x='scene', y='VMAF', hue='a')
sns.move_legend(ax, 'lower left')
plt.savefig('./figs/a_VMAF.eps', bbox_inches = 'tight')
plt.show()

ax = sns.barplot(data=selected, x='scene', y='PSNR', hue='a')
sns.move_legend(ax, 'lower left')
plt.savefig('./figs/a_PSNR.eps', bbox_inches = 'tight')
plt.show()
#%%
# X
# ---------------------------------------------------------------------------- #
...
plt.ylim(0.7, 1.0)
sns.move_legend(ax, 'lower left')
plt.savefig('./figs/comp_SSIM.eps', bbox_inches = 'tight')
plt.show()

ax = sns.barplot(data=selected, x='scene', y='VMAF', hue='maxNodes')
# plt.ylim(0.5, 1.0)
sns.move_legend(ax, 'lower left')
plt.savefig('./figs/comp_VMAF.eps', bbox_inches = 'tight')
plt.show()

ax = sns.barplot(data=selected, x='scene', y='PSNR', hue='maxNodes')
plt.ylim(10, 40)
sns.move_legend(ax, 'lower left')
plt.savefig('./figs/comp_PSNR.eps', bbox_inches = 'tight')
plt.show()
#%%
# traffic reduction
...
with sns.axes_style("white"):
    ...
    labels = ['Color View', 'Depth View', 'Probing View', 'Novel View']
    legendSet = False
    for m in ms:
        bottom = np.zeros((3,))
    ...
    plt.ylabel('Average Traffic per Client (Mbps)') # was MBps
    # plt.legend(loc='upper right', bbox_to_anchor=(1.0, 1.0))
    plt.legend(bbox_to_anchor=(0.7, 1.0))
    sns.despine()
plt.savefig('./figs/traffic.eps', bbox_inches = 'tight')

#%%
# X
# ---------------------------------------------------------------------------- #
#                             Target with skip line                            #
# ---------------------------------------------------------------------------- #
# stack = tex - dep - aux - tv
import seaborn as sns
import matplotlib.pyplot as plt
...
for m in ms:
    ...
    for tp in tps:
        data[m][tp] = [] # 3 scenes
        for scene in scenes:
            data[m][tp].append(stat[scene][m][tp] / 1e6 / 30 / N_USERS*8)
print(data)
'''
{'0': {'tex': [0.0, 0.0, 0.0], 'dep': [0.0, 0.0, 0.0], 'aux': [0.0, 0.0, 0.0], 'tv': [12.052356145833333, 13.135758827083333, 14.447326295833333]}, '0.01': {'tex': [0.18341903750000002, 0.18983192083333333, 0.2070913395833333], 'dep': [0.045084197916666666, 0.04825613958333333, 0.046935377083333334], 'aux': [0.004454925, 0.005357395833333333, 0.004947704166666666], 'tv': [0.0, 0.0, 0.0]}, '0.02': {'tex': [0.36526899791666667, 0.3813589125, 0.4155154833333333], 'dep': [0.09017103333333334, 0.09620914166666666, 0.09377902499999999], 'aux': [0.008994906249999999, 0.010677737500000001, 0.00996335625], 'tv': [0.0, 0.0, 0.0]}, '0.03': {'tex': [0.5469851083333332, 0.5731861208333334, 0.62545638125], 'dep': [0.13512209375, 0.14460206458333333, 0.14085742083333336], 'aux': [0.013418983333333334, 0.01615781875, 0.015019389583333332], 'tv': [0.0, 0.0, 0.0]}, '0.04': {'tex': [0.7304166770833334, 0.764081625, 0.8319984041666667], 'dep': [0.18020219583333336, 0.19341894791666667, 0.18779064583333333], 'aux': [0.01794797916666667, 0.021739320833333336, 0.020089091666666666], 'tv': [0.0, 0.0, 0.0]}, '0.05': {'tex': [0.91582698125, 0.9538710520833333, 1.0415600583333333], 'dep': [0.22597580208333332, 0.24216209791666668, 0.23467210416666667], 'aux': [0.02251355, 0.027337354166666668, 0.025120514583333333], 'tv': [0.0, 0.0, 0.0]}}
'''
...
bs = 8.0
ts = 80
ax1.set_ylim(ts,ts+40)
ax2.set_ylim(0,bs)
ax2.set_xticks(bar_positions['0.03'])
...
with sns.axes_style("white"):
    ...
    for m in ms:
        ...
        for i, tp in enumerate(tps):
            print(f"{m} {i} {tp}")
            if legendSet == False:
                bar1 = plt.bar(bar_positions[m], data[m][tp], bar_width,
                    	color=colors[i], label=labels[i], bottom=bottom)
                plt.subplot(211)

                for bar in bar1:
                    posx = bar.get_x() + bar.get_width()/2.
                    if bar.get_height() > ts:
                        ax1.plot((posx-3*d, posx+3*d), (- d, + d), color='k', clip_on=False,
                                transform=ax1.get_xaxis_transform())

                
                bar2 = plt.bar(bar_positions[m], data[m][tp], bar_width,
                    	color=colors[i], label=labels[i], bottom=bottom)
                
                for bar in bar2:
                    posx = bar.get_x() + bar.get_width()/2.
                    if bar.get_height() > bs:
    ...
    # plt.xticks(bar_positions['0.03'], scenes)
    # ax2.set_xticks(bar_positions['0.03'], scenes)
    # ax2.set_xticks(bar_positions['0.03'])
    # for ax in f.get_axes():
    #     ax.label_outer()
    
    f.text(-0.010, 0.5, 'Average Traffic per Client (Mbps)', va='center', rotation='vertical')
    # plt.ylabel('Average Traffic per Client (Mbps)') # was MBps
    # plt.legend(loc='upper right', bbox_to_anchor=(1.0, 1.0))
    plt.legend(loc='upper right', ncol=2, bbox_to_anchor=(0.95, 2))
    # sns.despine()
plt.savefig('./figs/traffic.eps', bbox_inches = 'tight')
#%%
# X
...
# 	# plt.tight_layout()
# 	sns.despine()
# plt.savefig('./figs/traffic_noTargetView.eps', bbox_inches='tight')
# plt.savefig('./figs/traffic_noTargetView.eps')
...
plt.savefig('./figs/correlation.eps', bbox_inches = 'tight')
# %%
expJ = json.load(open('qual_merge.json', 'r'))
...
with sns.axes_style("white"):
    ...
    # sns.despine()
plt.savefig('./figs/time_comp.eps', bbox_inches = 'tight')
#%%
# Fig 9
# ---------------------------------------------------------------------------- #
#                             Target with total traffic          #
# ---------------------------------------------------------------------------- #
# stack = tex - dep - aux - tv
import seaborn as sns
import matplotlib.pyplot as plt
...
for m in ms:
    ...
    for tp in tps:
        ...
        for scene in scenes:
            # was
            # data[m][tp].append(stat[scene][m][tp] / 1e6 / 30 / N_USERS*8)
            data[m][tp].append(stat[scene][m][tp] / 1e6 *8)
# hardcoded: 20094 KB for 30 seconds in ultra-fast encoding
print(data)
# data are loaded in Mb for 30 seconds
'''
{'0': {'tex': [0.0, 0.0, 0.0], 'dep': [0.0, 0.0, 0.0], 'aux': [0.0, 0.0, 0.0], 'tv': [12.052356145833333, 13.135758827083333, 14.447326295833333]}, '0.01': {'tex': [0.18341903750000002, 0.18983192083333333, 0.2070913395833333], 'dep': [0.045084197916666666, 0.04825613958333333, 0.046935377083333334], 'aux': [0.004454925, 0.005357395833333333, 0.004947704166666666], 'tv': [0.0, 0.0, 0.0]}, '0.02': {'tex': [0.36526899791666667, 0.3813589125, 0.4155154833333333], 'dep': [0.09017103333333334, 0.09620914166666666, 0.09377902499999999], 'aux': [0.008994906249999999, 0.010677737500000001, 0.00996335625], 'tv': [0.0, 0.0, 0.0]}, '0.03': {'tex': [0.5469851083333332, 0.5731861208333334, 0.62545638125], 'dep': [0.13512209375, 0.14460206458333333, 0.14085742083333336], 'aux': [0.013418983333333334, 0.01615781875, 0.015019389583333332], 'tv': [0.0, 0.0, 0.0]}, '0.04': {'tex': [0.7304166770833334, 0.764081625, 0.8319984041666667], 'dep': [0.18020219583333336, 0.19341894791666667, 0.18779064583333333], 'aux': [0.01794797916666667, 0.021739320833333336, 0.020089091666666666], 'tv': [0.0, 0.0, 0.0]}, '0.05': {'tex': [0.91582698125, 0.9538710520833333, 1.0415600583333333], 'dep': [0.22597580208333332, 0.24216209791666668, 0.23467210416666667], 'aux': [0.02251355, 0.027337354166666668, 0.025120514583333333], 'tv': [0.0, 0.0, 0.0]}}
'''
...
with sns.axes_style("white"):
    ...
    labels = ['Novel View', 'Source View (RGB)', 'Source View (D)', 'Probing View']
    legendSet = False
    for m in ms:
        bottom = np.zeros((3,))
        for i, tp in enumerate(tps):
    ...
    f.text(-0.02, 0.5, 'Total Bandwidth (Mb)', va='center', rotation='vertical')
    # plt.ylabel('Average Traffic per Client (Mbps)') # was MBps
    # plt.legend(loc='upper right', bbox_to_anchor=(1.0, 1.0))
    plt.legend(loc='upper right', ncol=2, bbox_to_anchor=(0.95, 2))
    # sns.despine()
plt.savefig('./figs/traffic_total_30sec.eps', bbox_inches = 'tight')
# %%
# No use func
